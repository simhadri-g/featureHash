...
def has_for_hlo(hidden_layer_output,X,comp_ratio): 
    ...
    return hashed_hlo
            #w1_virtual[i][j] = w1_real[k]
rmserror= [[] for zi in range(5) ]
z = 0
for comp in range(1,9,2):
    ...
    #    #print("ola", ola)
        ...
        rmserror[z].append(sk.mean_squared_error(yp, predicted_output))
        #Backpropagation
    ...
    z+=1
    print("predicted out put with com:",comp,"=",predicted_output)
xaxis= [l for l in range(epochs)]
fig, axs = plt.subplots(2,2)
fig.suptitle('Training error:1000 epochs')
pl12 =  0
for zi in range(2):
    for ki in range(2):      
        axs[zi][ki].set_title("compression ="+str(2*pl12+1))
        
        
        axs[zi][ki].plot( xaxis,rmserror[pl12])
        pl12+=1
